...
#        gcode.append(f'G01 Z0.00 ; Lower Z to cutting height') noko
        ...
        for x_px, y_px in points:
            ...
            xdelta=xold-x_mm
            ydelta=yold-y_mm
            if xdelta!=xdeltaold or ydelta!=ydeltaold: 
                if hanta: gcode.append(f'G01 X{xold:.3f} Y{yold:.3f} {FEED_RATE} ; HÄNTÄ {laskuri}')
                gcode.append(f'G01 X{x_mm:.3f} Y{y_mm:.3f} {FEED_RATE}')
                hanta=False
                laskuri=0
            else:
#                gcode.append(f'; IIK X{x_mm:.3f} Y{y_mm:.3f} {xdelta} {ydelta}')
                laskuri+=1
                hanta=True
            xdeltaold=xdelta
            ydeltaold=ydelta
...
try:
    ...
    # Use simple binary thresholding to avoid "double cut" effect
    ...
    # The outer level is the external boundaries, and the second level is the boundaries of the holes.
